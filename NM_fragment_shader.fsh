...
void main(void)
{
    ...
    vec3 t = normalize(normale_Matrix * tangent);
    vec3 b = cross(normale, t);

    vec3 tmpVec = gl_LightSource[0].position.xyz - vertex;
    mat3 TBN = transpose(mat3(t,b,normale));

    vec3 lightVec;
    lightVec.x = dot(tmpVec, t);
    lightVec.y = dot(tmpVec, b);
    lightVec.z = dot(tmpVec, normale);
    ...
    float diffuse = max(dot(lVec, n), 0.2 );

    vec4 vDiffuse = gl_LightSource[0].diffuse * gl_FrontMaterial.diffuse *
                                    diffuse;

    float specular = pow(clamp(dot(reflect(-lVec, n), vVec), 0.1, 1.0),
